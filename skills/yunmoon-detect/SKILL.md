...
표면적 유창성에 속지 않고(`<fluency_trap>` 참조) 표현·내용 차원의 단서에 주목하는 것.
...
신호를 합산해 **AI 가능성**과 **신뢰도**를 산정한다(`<scoring_rubric>` 참조). 표면 유창성·구조 완결성을
사람 증거로 오인하지 않는다(`<fluency_trap>`). 확정이 아니라 추정임을 유지한다.
</phase>
...
> 형식 점수(유창성·구조 완결성)는 **그 자체로 사람 증거가 아니다**(`<fluency_trap>`). "매끄러우니 사람"이라는 추론을 금한다.
...
- **유창성 함정**(`<fluency_trap>`)을 기억한다 — 매끄럽다고 사람, 어설프다고 AI가 아니다.
