...
- ✅ **11대 AI 티 분류 + 8대 번역투 유형** — 체계적 탐지·교정, 규정 근거와 함께 내역 제시
- ✅ **과잉·과소 양방향** — 장식·군더더기를 덜어내면서, 성분 생략·개조식으로 넘어가지 않게 되잡는다
...
**출력**: ① 한 줄 상태 `완료. 변경률 X% / 등급 Y / 자체검증 N/7 통과` ② 윤문본
...
| 11  | 과소 서술·의미 불명확 | 성분·조사·어미 생략, 개조식 종결, 비유 어휘 대치   |

1~10번은 말이 **넘쳐서** 생기는 티이고, 11번은 말이 **모자라서** 생기는 티다. 방향이 반대이므로 한쪽만 밀면
다른 쪽으로 넘어간다 — 그래서 보존 원칙에 '삭제 과교정 금지' 가드를 두었다.
...
│   │   └── references/
│   │       ├── ai-tell-taxonomy.md    # 11대 AI 티 분류 (humanize·detect 공유)
...
11번 카테고리(과소 서술·의미 불명확)와 '삭제 과교정 금지' 가드는 Claude Code output-style 플러그인
[`snflkd/fluent-korean`](https://github.com/snflkd/fluent-korean)(MIT)이 정리한 LLM 한국어 출력 특징을
윤문 관점으로 옮긴 것이다. 그 플러그인은 **Claude 자신의 한국어 출력**을 규율하고 yoonmoon은
**사용자가 준 텍스트**를 다듬으므로, 둘은 대체 관계가 아니라 함께 쓰는 관계다.
